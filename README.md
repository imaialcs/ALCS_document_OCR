...
### 不具合修正
- 特定のファイル（テーブル形式のドキュメント）を処理する際に、アプリケーションがクラッシュする問題を修正しました。
- 空白の列名がある場合に、列の表示/非表示チェックボックスが正しく機能しない不具合を修正しました。

### 機能改善
- ウィンドウサイズが小さい場合に、結果表示画面のヘッダーレイアウトが崩れる問題を修正しました。
- 列の表示/非表示を切り替えるUIで、空白の列名が「空白列」と表示されるように改善しました。
- アプリケーションのビルド構成を更新し、インストーラーのファイル名を `ALCS-Document-OCR-Setup-バージョン.exe` の形式に標準化しました。
